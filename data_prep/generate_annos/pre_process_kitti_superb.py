...
def create_data_info_pkl(data_root, data_type, prefix, label=True, db=False):   # data_root=data_to_superbai
    sep = os.path.sep
    print(f"Processing {data_type} data..")
    ids_file = os.path.join('/', *data_root.split('/'), f'{data_type}.txt') # train.txt / val.txt / test.txt 
    ...
    for id in tqdm(ids):    # ids : txt 파일 안에 있는 경로들을 포함한 부분들
        cur_info_dict={}
        data_key = id.split()[-1].split('/')[0]
        lidar_ts = os.path.basename(id.split()[-1]).split('.')[0]
        import pdb 
        try:
            image_id = find_closest_img(data_root, data_key, lidar_ts)
        except:
            pdb.set_trace()
        img_path = os.path.join(data_root, data_key, 'sample', f'{image_id}.jpg')
        lidar_path = os.path.join(data_root, data_key, 'sample', f'{lidar_ts}.avikus.pcd')            
        calib_path = os.path.join(data_root, data_key, f'calib_{data_key}.txt')
