...
| 1b | `convert_label_z_center_to_bottom.py` | `<data_name>/label/*.txt` for sessions without `tracklet_labels.xml` (i.e. SuperbAI) | `<data_name>/label/*.txt`, z shifted from bbox-center to bbox-bottom-center and dims swapped LWH -> WLH, in place | SuperbAI sessions only (marker files `.z_converted` / `.dims_swapped` make each conversion independently idempotent across re-runs; height read from field 10, empirically verified against real point-cloud extents) |
